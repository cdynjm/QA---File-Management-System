...
$COUNT_FOLDERS = 0;
global $PATH;
$SQL_COUNT = "SELECT * FROM files WHERE folder_id='$folder_id'";
...
$SQL_COUNT = "SELECT * FROM folders WHERE id='$folder_id'";
$SQL_COUNT_QUERY = mysqli_query($db, $SQL_COUNT);

if(mysqli_num_rows($SQL_COUNT_QUERY) > 0) {
    while($ROWS = mysqli_fetch_assoc($SQL_COUNT_QUERY)) {
        $PATH .= "".$ROWS['name']." Directory";
    }
}
?>
<style>
    ...
    <div class="body">
            ...
            <table>
                ...
                $sql = "SELECT * FROM files WHERE folder_id='$folder_id' ORDER BY date DESC";
    ...
    <div class="folders">
            <p><?php echo $PATH; ?></p>
            <?php 
                $sql = "SELECT * FROM folders WHERE parent_id='$folder_id' ORDER BY name ASC";
                $query = mysqli_query($db, $sql);
                $cnt = 1;
                if(mysqli_num_rows(($query)) > 0) {
                    while($rows = mysqli_fetch_assoc($query)) {
